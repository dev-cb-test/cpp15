...
/* #############################################################################################################
   ...
   #####                                                                                                   #####
   #####  OPTIONS:                                                                                         #####
   #####            -ap --application-layer-protocol <protocol> FTP | IMAP | POP3 | SMTP                   #####
   ...
   #####                          <login-username> - replaced by the -lu parameter value                   #####
   #####                          <login-password> - replaced by the -lp parameter value                   #####
   #####                          <buffer> - replaced by the -bc parameter value * -bs parameter value     #####   
   #####                          <remote-domain> - replaced by the -th parameter value (only domain)      #####
   #####                                                                                                   #####
   #####                                                                                                   #####
...
#define APP_BOVSTT_DEFAULT_USER_INPUT_PARAMETER_BUFFER_SIZE_LENGHT            1024
...
#define APP_BOVSTT_PARAMETER_OUTPUT_VERBOSE_ID                                "output-verbose"
#define APP_BOVSTT_PARAMETER_OUTPUT_VERBOSE_ALIAS_1                           "-ov"
#define APP_BOVSTT_PARAMETER_OUTPUT_VERBOSE_ALIAS_2                           "--output-verbose"
...
#define APP_BOVSTT_PARAMETER_TYPE_SPECIAL_ALPHANUMERIC                        "PARAMETER_TYPE_SPECIAL_ALPHANUMERIC"
...
#define APP_BOVSTT_WARNING_CONNECTION_KEEP_ALIVE                              "Possible buffer overflow vulnerability found or connection closed by remote host.\r\n\r\n\t Try a smaller buffer size length using the --buffer-size-length parameter."
...
string decodeApplicationLayerProtocolCommand(string sApplicationLayerProtocolCommand, cTypeUserInputParameters &oAppUserInputParameters, bool bFullDecode = true);
...
void printScreenAppVerbose(string sAppVerbose, int nEndCommandLine);
         ...
         if (oClientService != NULL) {
            ...
            if (FFile::fileExists(sApplicationLayerProtocolPathFilename)) {
               ...
               if (oClientService->connectSocket(oAppUserInputParameters.sTargetHostnameIP, oAppUserInputParameters.nTargetPort, oAppUserInputParameters.sCryptographicSecurityProtocol)) {
                  ...
                  if (oClientService->getReceiveOnConnect()) {
                     sLastRecvSocketData = oClientService->recvSocketData();
                  
                     if (oAppUserInputParameters.bOutputVerbose) printScreenAppVerbose(sLastRecvSocketData, oClientService->getEndCommandLine().length());
                  }
                  ...
                  for(;(((sApplicationLayerProtocolFileLineCommand = FFile::fileReadLine(&oInputFileStream)) != FFILE_READ_LINE_EOF) && (!bErrorProcessApplicationLayerProtocolFile));) {
                     ...
                     if (sApplicationLayerProtocolFileLineCommand.length()) {                     	
                        ...
                        if (bApplicationLayerProtocolFileLineProcessCommand) {
                           string sApplicationLayerProtocolFileLinePartialDecodedCommand = decodeApplicationLayerProtocolCommand(sApplicationLayerProtocolFileLineCommand, oAppUserInputParameters, false);

                           oStringTokens->clear();
                           oStringTokens->push_back(pair<string, string>("{1}", sApplicationLayerProtocolFileLinePartialDecodedCommand));     
...
void initializeAppUserInputParameters(cTypeUserInputParameters &oAppUserInputParameters) {
   oAppUserInputParameters.sApplicationLayerProtocol = GLOBAL_STRING_EMPTY;
   oAppUserInputParameters.sTargetHostnameIP = GLOBAL_STRING_EMPTY;
   ...
   oAppUserInputParameters.bOutputVerbose = false;
}
...
void initializeAppParameters(vector<cTypeParameter *> &oAppParameters, cTypeUserInputParameters &oAppUserInputParameters) {
   ...
   oAppParameter1->oValues.push_back(GLOBAL_APPLICATION_LAYER_PROTOCOL_IMAP);
   oAppParameter1->oValues.push_back(GLOBAL_APPLICATION_LAYER_PROTOCOL_POP3);
   oAppParameter1->oValues.push_back(GLOBAL_APPLICATION_LAYER_PROTOCOL_SMTP);
   ...
   oAppParameter5->sTypeValue = APP_BOVSTT_PARAMETER_TYPE_SPECIAL_ALPHANUMERIC;
   ...
   oAppParameter6->sTypeValue = APP_BOVSTT_PARAMETER_TYPE_SPECIAL_ALPHANUMERIC;
   ...
   /* Parameter: --output-verbose */
   cTypeParameter * oAppParameter9 = new cTypeParameter();
   
   oAppParameter9->sId = APP_BOVSTT_PARAMETER_OUTPUT_VERBOSE_ID;
   
   oAppParameter9->oAliases.push_back(APP_BOVSTT_PARAMETER_OUTPUT_VERBOSE_ALIAS_1);
   oAppParameter9->oAliases.push_back(APP_BOVSTT_PARAMETER_OUTPUT_VERBOSE_ALIAS_2);
   
   oAppParameter9->bAlone = false;
   oAppParameter9->bRequiredValue = false;
   oAppParameter9->bRequiredUserInput = false;
   oAppParameter9->bUserInput = false;

   oAppParameter9->pUserInputParameter = &oAppUserInputParameters.bOutputVerbose;
   
   /* Parameter: --credits */
   cTypeParameter * oAppParameter10 = new cTypeParameter();
	
   oAppParameter10->oAliases.push_back(APP_BOVSTT_PARAMETER_CREDITS_ALIAS_1);
   oAppParameter10->oAliases.push_back(APP_BOVSTT_PARAMETER_CREDITS_ALIAS_2);
   ...
   oAppParameter10->pFunction = printScreenAppCredits;
   
   /* Parameter: --version */
   cTypeParameter * oAppParameter11 = new cTypeParameter();
	
   oAppParameter11->oAliases.push_back(APP_BOVSTT_PARAMETER_VERSION_ALIAS_1);
   oAppParameter11->oAliases.push_back(APP_BOVSTT_PARAMETER_VERSION_ALIAS_2);
   
   oAppParameter11->bAlone = true;
   oAppParameter11->bRequiredValue = false;
   oAppParameter11->bRequiredUserInput = false;
   oAppParameter11->bUserInput = false;
   
   oAppParameter11->pFunction = printScreenAppVersion;
   ...
   oAppParameters.push_back(oAppParameter11);
}
...
string decodeApplicationLayerProtocolCommand(string sApplicationLayerProtocolCommand, cTypeUserInputParameters &oAppUserInputParameters, bool bFullDecode) {
   vector< pair<string, string> > * oStringTokens = new vector< pair<string, string> >();
   ...
   if (bFullDecode) {
      ...
      sDecodeBuffer = oAppUserInputParameters.cBufferCharacter + string("x") + FCast::castNumberToString(oAppUserInputParameters.nBufferSizeLenght);						   
   }
   ...
   if (FFormat::isFormatHostname(oAppUserInputParameters.sTargetHostnameIP)) oStringTokens->push_back(pair<string, string>(GLOBAL_STF_FILE_ENCODE_NOTATION_REMOTE_DOMAIN, FString::getFromFirstToken(oAppUserInputParameters.sTargetHostnameIP, ".")));
   else oStringTokens->push_back(pair<string, string>(GLOBAL_STF_FILE_ENCODE_NOTATION_REMOTE_DOMAIN, oAppUserInputParameters.sTargetHostnameIP));
         ...
         if (bNextParameter) {
            if ((isValidAppParameterKeyWord(oAppInputParameters[i], false, oAppParameters)) && (!isValidAppParameterKeyWord(oAppInputParameters[i], true, oAppParameters))) {
               ...
               if (oAppParameter->bRequiredValue) bNextParameter = false;
                  ...
                  *((bool *) oAppParameter->pUserInputParameter) = true;
...
bool isValidAppParameterValue(string sAppUserInputParameter, cTypeParameter * oAppParameter) {
   ...
   if (oAppParameter != NULL) {
      ...
      else if (oAppParameter->sTypeValue == APP_BOVSTT_PARAMETER_TYPE_SPECIAL_ALPHANUMERIC) {
         for(int i = 0; ((i < sAppUserInputParameter.length()) && (!bError)); i++) {
            if (!((sAppUserInputParameter[i] >= 33) && (sAppUserInputParameter[i] <= 126))) bError = true;     
         }
...
void printScreenAppVerbose(string sAppVerbose, int nEndCommandLine) {
   sAppVerbose = sAppVerbose.substr(0, sAppVerbose.length() - nEndCommandLine);
   
   if (sAppVerbose.length() > 0) cout << endl << "[ " << "\033[1;32m" << sAppVerbose << "\033[0m" << " ]" << endl;
}
